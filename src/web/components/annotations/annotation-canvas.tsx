 ...
 */
function isPointInShape(point: Point, shape: AnnotationShape): boolean {
  const { x, y } = point;
  ...
  const drawAnnotations = useCallback(
    (ctx: CanvasRenderingContext2D, currentShape?: Partial<AnnotationShape>) => {
      ...
      annotations.forEach((annotation) => {
        drawShape(ctx, annotation, width, height, annotation.id === selectedId);
      });

      // Draw current shape being drawn
      if (currentShape) {
        drawShape(ctx, currentShape as AnnotationShape, width, height, false, true);
      }
    },
